...
export function useGame() {
  ...
  /**
   * `idemKey` must be a stable key owned by the caller (see
   * CashoutFlow.tsx for its lifecycle). `contextId` must be the real
   * `game_state.updated_at` value the caller observed for the pot it's
   * trying to cash out — sent through to the server as-is (no
   * reformatting, no local Date parsing) so Postgres compares it as a
   * real `timestamptz`, not a string. The server independently verifies
   * this context against the game_state row it locks; a stale or
   * mismatched context is rejected server-side regardless of what the
   * frontend believed locally.
   */
  const cashout = useCallback(async (gameId: string, idemKey: string, contextId: string) => {
    setCashingOut(true);
    setError(null);
