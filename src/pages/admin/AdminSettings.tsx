...
import { Save, Check, Settings, Puzzle, BookOpen, Coins, AlertTriangle, Info, Bell } from 'lucide-react';
import { useAdmin } from '../../hooks/useAdmin';
import LoadingSpinner from '../../components/ui/LoadingSpinner';
import AdminPushTestPanel from './AdminPushTestPanel';

export type SettingsSection = 'system' | 'economy' | 'reminders' | 'push' | 'integrations' | 'puzzle' | 'onboarding' | 'all';
...
// STS Reactivation System (native Web Push) — a separate, deliberately
// distinct settings group from REMINDER_KEYS above (which is the
// existing email/SMS/GHL-based system). See PROJECT_CHANGELOG.md
// "Reactivation System Phase 1".
const PUSH_KEYS = [
  'push_global_enabled', 'push_next_day_enabled', 'push_next_day_delay_minutes',
  'push_last_call_enabled', 'push_last_call_minutes_before_cutoff',
  'push_min_spacing_hours', 'push_max_per_player_per_day',
  'push_next_day_title', 'push_next_day_body_streak', 'push_next_day_body_no_streak',
  'push_last_call_title_streak', 'push_last_call_body_streak',
  'push_last_call_title_no_streak', 'push_last_call_body_no_streak',
  'push_click_route', 'push_test_mode',
];
...
const SETTING_META: Record<string, { label: string; description: string; type: 'number' | 'boolean' | 'json' | 'text'; min?: number; max?: number; step?: string }> = {
  ...
  // ── STS Reactivation System (native Web Push) ──────────────────────────
  push_global_enabled: {
    label: 'Push Notifications Enabled',
    description: 'Master switch. When off, the scheduler sends nothing regardless of any other setting below.',
    type: 'boolean',
  },
  push_next_day_enabled: {
    label: 'Next-Day Reminder Enabled',
    description: 'Sends once a new challenge is available, to players who played the previous game day.',
    type: 'boolean',
  },
  push_next_day_delay_minutes: {
    label: 'Next-Day Send Delay (minutes after daily reset)',
    description: 'Minutes to wait after the Madrid daily reset before the next-day reminder becomes eligible to send.',
    type: 'number',
    min: 0,
    max: 1440,
  },
  push_last_call_enabled: {
    label: 'Last-Call Reminder Enabled',
    description: "Sends to players who haven't played yet as today's opportunity approaches its close.",
    type: 'boolean',
  },
  push_last_call_minutes_before_cutoff: {
    label: 'Last-Call Window (minutes before cutoff)',
    description: 'How many minutes before the Madrid daily cutoff the last-call reminder becomes eligible.',
    type: 'number',
    min: 5,
    max: 720,
  },
  push_min_spacing_hours: {
    label: 'Minimum Spacing Between Notifications (hours)',
    description: 'Minimum gap between any two STS push sends to the same player, regardless of type.',
    type: 'number',
    min: 0,
    max: 24,
  },
  push_max_per_player_per_day: {
    label: 'Maximum Notifications Per Player Per Day',
    description: 'Hard cap across both notification types combined.',
    type: 'number',
    min: 1,
    max: 5,
  },
  push_next_day_title: { label: 'Next-Day Title', description: 'Notification title for the next-day reminder.', type: 'text' },
  push_next_day_body_streak: { label: 'Next-Day Body (active streak)', description: 'Shown when the player has an active streak.', type: 'text' },
  push_next_day_body_no_streak: { label: 'Next-Day Body (no streak)', description: 'Shown when the player has no active streak.', type: 'text' },
  push_last_call_title_streak: { label: 'Last-Call Title (active streak)', description: 'Shown when current_streak > 0.', type: 'text' },
  push_last_call_body_streak: { label: 'Last-Call Body (active streak)', description: 'Streak-risk language — only ever shown when a streak is genuinely active.', type: 'text' },
  push_last_call_title_no_streak: { label: 'Last-Call Title (no streak)', description: 'Shown when current_streak === 0.', type: 'text' },
  push_last_call_body_no_streak: { label: 'Last-Call Body (no streak)', description: 'Never implies a streak is at risk.', type: 'text' },
  push_click_route: {
    label: 'Notification Click Route',
    description: 'In-app route opened on click. Must be one of the allow-listed routes enforced by the service worker and Edge Functions — changing this here does not bypass that allow-list.',
    type: 'text',
  },
  push_test_mode: {
    label: 'Test Mode',
    description: 'When on, admin test sends are clearly labeled as tests and never count toward a player\u2019s daily notification limit (this is already true regardless of this flag — it exists for future stricter gating).',
    type: 'boolean',
  },
};
...
export default function AdminSettings({ section = 'all' }: { section?: SettingsSection }) {
  ...
  const pushSettings = settings.filter((s) => PUSH_KEYS.includes(s.key));
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Reactivation Notifications (native Web Push) ────────────────────
          Deliberately separate from Reminders above (that group is the
          existing email/SMS/GHL system) — see PROJECT_CHANGELOG.md
          "Reactivation System Phase 1". */}
      {show('push') && pushSettings.length > 0 && (
        <PushSection
...
// ── Reactivation Notifications (Web Push) Section ───────────────────────────

function PushSection({ settings, editValues, setEditValues, saving, savedKey, onSave }: RemindersSectionProps) {
  if (settings.length === 0) return null;
  return (
    <section>
      <SectionHeader icon={<Bell className="h-4 w-4 text-torch-ember" strokeWidth={1.5} />} title="Reactivation Notifications" />
      <div className="border border-moss-dark/25 bg-ritual-surface/20 divide-y divide-moss-dark/15">
          ...
          return (
            <div key={setting.key} className="px-4 py-3">
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-xs font-medium text-bone">{meta?.label ?? setting.key}</span>
                    <span className="text-[11px] font-mono text-bone-faint">{setting.key}</span>
                  </div>
                  {meta?.description && (
                    <p className="text-[12px] text-bone-faint leading-relaxed mb-2">{meta.description}</p>
                  ...
                  {isBoolean ? (
                    ...
                    <input
                      ...
                      value={editValues[setting.key] ?? ''}
                      onChange={(e) => setEditValues((p) => ({ ...p, [setting.key]: e.target.value }))}
                      className="ritual-input w-full max-w-md font-mono text-xs"
                      onKeyDown={(e) => e.key === 'Enter' && onSave(setting.key)}
                    />
                  )}
                </div>
                <div className="pt-6 flex-shrink-0">
