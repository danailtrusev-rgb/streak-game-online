...
export default function HomePage() {
  ...
  const prevMaxStreakRef = useRef<number | null>(null); // best streak BEFORE the in-flight play — see handleConfirmStake; null = unknown, never inferred as 0
  const prevQualificationPointsRef = useRef<number | null>(null); // qualification points BEFORE the in-flight play — see handleConfirmStake
  ...
  const handleConfirmStake = async (tier: number) => {
    setPhase('idle');
    // Snapshot BEFORE this play resolves. game_state is present here (we're
    // mid-game, not loading), so max_streak is a real, known number — this
    // is a genuine previous value, not a stand-in for missing data.
    prevMaxStreakRef.current = game_state?.max_streak ?? null;
    prevQualificationPointsRef.current = qualification?.total_points ?? null;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {phase === 'result' && lastResult && (
        <ResultModal
          ...
          potCents={potCents}
          previousBestStreak={prevMaxStreakRef.current}
          qualification={qualification}
          previousQualificationPoints={prevQualificationPointsRef.current}
          cashoutContextId={game_state?.updated_at ?? ''}
        />
      )}
    </div>
  );
