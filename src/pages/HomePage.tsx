...
export default function HomePage() {
  ...
  const prevMaxStreakRef = useRef(0); // best streak BEFORE the in-flight play — see handleConfirmStake
  ...
  const handleConfirmStake = async (tier: number) => {
    setPhase('idle');
    prevMaxStreakRef.current = game_state?.max_streak ?? 0; // snapshot BEFORE this play resolves
