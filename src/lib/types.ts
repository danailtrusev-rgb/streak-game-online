...
export interface GameState {
  ...
  /**
   * Real, server-generated "version" for the current eligible state —
   * changes on every play and every cashout. Used as the cashout-context
   * identifier (see src/lib/cashoutIdempotency.ts) so a stale idempotency
   * key from a previous, already-resolved cashout can never be reused for
   * a new one. `null` only when the player has no game_state row yet.
   */
...
export interface CashoutResult {
  ...
  played_today: boolean;
  /** Real wallet_ledger row id — the confirmed transaction reference. */
  transaction_id: string;
  /** Always 'EUR' today — this project has no multi-currency support anywhere in the schema. */
  currency: string;
  /** True when this response is a replay of an already-completed transaction for the same idempotency key, not a new payout. Informational only — the frontend treats both cases identically (real, server-confirmed success). */
