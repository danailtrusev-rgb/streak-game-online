 ...
 * Fields left undefined simply disable the categories that need them
 * (e.g. no qualification object => qualification-progress messages never
 * get selected) rather than being backfilled with placeholder numbers.
 */
...
function isQualificationRelevant(ctx: ResultMessageContext): boolean {
  if (!ctx.qualification) return false;
  return ctx.qualification.points < ctx.qualification.pointsThreshold;
}
...
/** Simple deterministic pick so the same play always renders the same variant, but different plays vary. */
...
function buildVars(ctx: ResultMessageContext): Record<string, string | number> {
  return {
    streak: ctx.streak,
    nextMilestoneDay: ctx.nextMilestoneDay,
    timeUntilNextPlay: ctx.timeUntilNextPlay,
    qualificationPoints: ctx.qualification?.points ?? '',
    qualificationThreshold: ctx.qualification?.pointsThreshold ?? '',
  };
}
