...
interface OperatorContactFormProps {
  /** True only in the static Bolt preview build — never calls the real API. */
  previewMode?: boolean;
}

export default function OperatorContactForm({ previewMode = false }: OperatorContactFormProps) {
  const [form, setForm] = useState(initialForm);
  const [status, setStatus] = useState<'idle' | 'submitting' | 'sent' | 'unavailable' | 'error' | 'preview_disabled'>('idle');
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (previewMode) {
      // Never call /api/operator-contact from the static preview — there's
      // no backend running on Bolt Hosting to receive it, and we must not
      // fake a successful submission.
      setStatus('preview_disabled');
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 520, margin: '0 auto' }}>
      ...
      {status === 'preview_disabled' && (
        <p role="status" style={{ fontSize: 13, color: '#D4A020', fontFamily: "'Inter', system-ui, sans-serif" }}>
          Preview only — form delivery will be enabled on the production partner environment.
        </p>
      )}
      {status === 'unavailable' && (
