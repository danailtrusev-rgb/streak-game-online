...
interface ResultModalProps {
  ...
  onCashout?:  () => void;
  cashingOut?: boolean;
  /** Streak the player held immediately before this play — used to detect a new personal best. */
  ...
  const message = useMemo<ReturnType<typeof selectResultMessage>>(() => {
    const ctx: ResultMessageContext = {
      ...
      qualification: qualification
        ? { points: qualification.total_points, pointsThreshold: qualification.sat_pts_threshold }
        : null,
      // play_id makes the variant stable for this specific result if the
      // modal re-renders, but still varies play to play.
      variantSeed: result.play_id,
    };
