/*
  # Fix guest merge RPCs with correct table names

  The initial migration used assumed table names. This migration replaces both RPCs
  with corrected versions using the actual schema:
  - wallets → wallet_balance_cache
  - wallet_entries → wallet_ledger
  - qualification_progress → weekly_qualification_status (week_start → week_start_date)
*/
...
BEGIN
  ...
  PERFORM 1 FROM public.guest_account_merges
  WHERE guest_user_id = p_guest_user_id AND status = 'completed'
